  ...
  inFlightRefresh = (async () => {
    ...
    for (const tender of deduped) {
      ...
      if (prev && prev.pdfAnalysis) {
        ...
        if (prev.pdfAnalysis.relevanceFit) {
          tender.actionHint = prev.pdfAnalysis.relevanceFit.slice(0, 200);
        }
      }
    }
...
function verifyCandidates(snapshot, candidates) {
  ...
  return candidates
    ...
    .filter((candidate) => {
      ...
      // Global portals only show active tenders/notices on their listing pages — skip date check
      const skipDateCheck = ['epads-federal', 'world-bank', 'bangladesh-cptu', 'kenya-ppra', 'afdb'].includes(snapshot.source.id);
...
function normalizeTender(candidate, snapshot) {
  ...
  return {
    ...
    country: clean(candidate.country) || snapshot.source.country,
